...
2. **모델 탭 = `gpu-models-ver2`(data05 전용)** — fleet 모델 뷰로 유지·라벨. 노드 스코핑 없음(콘솔은 'GPU' 라벨 첫 탭에만 DCGM instance 주입 → `findGpuTab`이 `keiwi-gpu`를 선택, '모델' 탭은 미주입).
...
```bash
# 1) keiwi-gpu 대시보드 프로비저닝(라이브 Grafana는 바인드 마운트 없음 → docker cp)
...
# 2) .env.local GRAFANA_DASHBOARD_UID 탭 구성(시스템 | GPU=keiwi-gpu | 모델=gpu-models-ver2)
#    GPU 라벨은 keiwi-gpu에, 기존 gpu-models-ver2는 '모델'로 재라벨.
