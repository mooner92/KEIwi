...
# tier 0 · actions [] — **의도된 빈 화이트리스트다.**
#   이 문서의 명령은 전부 `<user>`·`<ip>`·`<port>`·`dataNN` 자리표시자를 사람이 채워야 하고,
#   그 값들은 §13에 따라 레포에 없다. 인자 검증을 코드로 할 수 없는 명령은 화이트리스트에
#   올릴 수 없다(spec §0-2 "검증된 인자"). 게다가 온보딩은 알림 대응이 아니라 **계획 작업**이라
#   자동화의 대상이 아니다 — L1은 이 문서를 인용할 수는 있어도 조치 블록을 만들지 않는다.
tier: 0
actions: []
---
