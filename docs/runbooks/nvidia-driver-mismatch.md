...
category: gpu
severity: critical
# alerts: [] — 탐지 알림(`node_nvidia_version_mismatch` 기반)이 아직 없다. 게이트는 WARN으로
#   통과시킨다(hardware-ops T0-3 계약). 알림 신설 시 여기에 alertname을 적는다.
alerts: []
service: nvidia
affected_nodes: [data05]
last_verified: 2026-08-06
# tier 0 = 자동 조치 후보 아님. 유일한 조치가 **노드 재부팅**이라 blast radius가 호스트 전체이고
#   (data05는 관제 스택 호스트), 되돌릴 수 없으며 사용자 작업·GPU 프로세스를 전부 끊는다.
#   L1이 고를 수 있는 목록에 들어가서는 안 되므로 actions를 비운다(게이트 A8).
tier: 0
actions: []
---
