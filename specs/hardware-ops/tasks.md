...
- [~] **T0-7** `[server]` (M) **day-1 오발화 후보 정리** — 2026-07-30 전 항목 진단 완료, 일부 실행 완료. 항목별 판정:
  ...
  - [ ] **vllm `:8010` down** — 원인은 기동 실패가 아니라 **T0-4 드라이버 mismatch 그 자체**: `NVML library version: 595.84` vs 구커널 모듈 → 신규 GPU 프로세스가 전부 `Failed to infer device type`으로 죽는다. `vllm-ocr-8010`(MineSweeper OCR)과 `vllm-qwen25-coder-32b`가 **auto-restart 무한 루프 중**(13초 CPU씩 소모). 8003이 사는 이유: 드라이버 교체 **전에** 이미 GPU를 잡은 프로세스라서. → 재부팅(T0-4) 전까지 루프 유닛 `systemctl stop` 권고, 근본 해결은 T0-4.
