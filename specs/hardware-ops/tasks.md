...
- [x] **T0-1** (S) `node-hygiene` role에 NVIDIA 정합성 4메트릭 블록 추가 — `node_nvidia_smi_ok` · `node_nvidia_kernel_module_version` · `node_nvidia_userspace_version` · `node_nvidia_version_mismatch`. **완료 2026-08-06** — data05 로컬 렌더 실행으로 4메트릭 출력 확인(595.84/595.84, mismatch=0). 검증: AC-3-3 (AC-3-2는 2026-08-06 개정 참조 — T0-4가 먼저 실행돼 "전 1" 시계열 확보 불가)
- [ ] **T0-2** `[server]` (S) T0-1 배포 — `ansible-playbook playbooks/agents.yml --tags node-hygiene --check` → 실적용. **선행: T0-1.** 기대(2026-08-06 개정): 전 GPU 노드 `node_nvidia_version_mismatch = 0` · `node_nvidia_smi_ok = 1` — 수복이 먼저 일어나 "=1 관측"은 소실, 목적은 **재발 감시**로 전환
- [x] **T0-3** (S) 런북 `docs/runbooks/nvidia-driver-mismatch.md` 생성 — **완료 2026-08-06.** 증상(exit 18·비대칭 생존·auto-restart 루프) → 판별 3줄 → 원인 → 조치(모델 교체 주의 포함) → 예방(ADR-0020)
- [x] **T0-4** `[server]` (M) **data05 재부팅으로 드라이버 수복 — 완료 2026-08-06**(사용자 직접 재부팅). 사후 검증 통과: `nvidia-smi` 정상(595.84 정합) · CDI 재생성(`/var/run/cdi/nvidia.yaml` 06:50) · `gpu_vram_total_bytes` data05 2장 복귀(ADR-0013 판정 회복) · vllm-qwen25-coder-32b 정상 기동. **편차 기록**: ① 탐지(T0-1/T0-2) 배포 전에 재부팅돼 mismatch=1→0 시계열 미확보(경고 블록의 순서 원칙 위반이 실제로 발생 — AC-3-2 개정) ② Q9 확인됨: `:8003` 모델 Qwen3-Coder-30B → **Qwen2.5-Coder-32B** 교체, `.env.local` `VLLM_MODEL` 교정 + 콘솔 재시작으로 어시스턴트 200 복구 ③ `:9836` 고아는 PM2 덤프에서 삭제 처분 ④ `vllm-ocr-8010`은 disabled 유지(필요 시 수동 기동) ⑤ PM2 startup 미등록이 발견돼 `pm2-mooner92.service` 등록(재부팅 회복력 보강)
