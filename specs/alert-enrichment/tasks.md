...
- [ ] **T-E2-1** (S) 전 규칙에 annotation 3종 추가 — `__dashboardUid__`(system-v3/gpu-v3/logs-v3 규칙별 배정), `drilldown_url`(var 후보 3종 미러링), `console_url`(`alert=·node=·mount=·from=` 파라미터, 한국어 없음). spec §2.2 D2-1. **선행: T-E1-1.** 검증: AC-E2-1
