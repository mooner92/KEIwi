...
> 2026-08-04 · 갱신 2026-08-05(§3 L2 확정). 상태: **L1·L2 구현 완료 · L3 미착수**. 권위: 헌장(§11 생성·사람 적용 · §12 라이브 직접수정 금지 · §I-1 온프레미스 · §13 시크릿 레포 밖 · §15 노이즈 최소 · §16 멱등). 배경·근거·비교표는 [README](./README.md).
>
...
> 먼저 고친다**(§7 SDD).
>
> 설계 원칙 한 줄: **감지는 통계(alerting SoT), 진단은 LLM(제안), 실행은 결정론(런북), 승인·개시는 사람(서버 셸).**
...
5. **불변 감사.** 제안·근거번호·승인자·실행·결과·롤백을 **로컬 append-only 원장**(`/var/log/keiwi/remediation.jsonl` · O_APPEND+fsync · 0600)에 남기고, filebeat가 그것을 OpenSearch `keiwi-remediation-*`로 실어 나른다. **쓰기는 로컬(강한 보장), 조회는 OpenSearch(편의)** — 이유는 §3.6-2(ADR-0026). 1인 운영의 유일한 사후검증 근거.
...
**tier 실측 분포** [2026-08-05, 런북 16종] — `check-runbook-actions.sh`가 출력한다:
`t0=4 · t1=10 · t2=1 · t3=1` (actions 64개). 즉 **런북의 88%가 제안 이하**이고,
**L2가 실제로 실행할 수 있는 것은 런북 2편(`log-ingestion-stopped` t3 · `node-hygiene-stale` t2)의
조치 7개뿐**이다. 위 표의 예측과 일치한다.

> 08-04 측정치(`t1=9 t3=2`)와 다른 이유: `disk-usage-high`가 `alerts` 소유권을 `disk-pressure`로
> 넘기면서 tier 3 → 1로 내려갔다(A9 — 도달 불가한 자동 정책 금지). **야심이 아니라 전제가
> tier를 정한다**는 §1 NOTE의 두 번째 사례다.
...
**상한 선언**이다(L2는 [ADR-0026](../../docs/decisions/0026-auto-remediation-ladder.md) **채택됨**, L3는 ADR-0027(신설 예정) 게이트 뒤).

| tier | 뜻 |
|---|---|
| **0** | 사람 전용. 어떤 자동 경로에도 오르지 않는다(재부팅·연구자 프로세스 kill 등 §4.5) |
| **1** | L1 제안까지. 실행 기능 없음 — 런북 16종 중 9종이 여기다 |
| **2** | L2 승인 후 실행 **대상**(ADR-0026 채택 — `remediation_l2.py`가 실제로 실행할 수 있다) |
| **3** | L3 사전승인 자동 **후보**(ADR-0027(신설 예정) + earned autonomy 후). tier 3도 **현재는 L2까지만** 실행된다 |
...
## 3. L2 — 승인 후 실행 ([ADR-0026](../../docs/decisions/0026-auto-remediation-ladder.md) 채택)

> 구현: `infra/alert-relay/remediation_l2.py` · 테스트 `test_remediation_l2.py`(50건) ·
> 게이트 `scripts/gates/check-remediation-l2.sh`(M1~M9).
> **이 절은 ADR-0026 채택 뒤 실측에 맞춰 갱신됐다(2026-08-05).** 초안에서 뒤집힌 판단 3건은
> §3.6에 사유와 함께 남긴다 — 스펙이 코드를 따라간 것이 아니라, 결정이 ADR에서 내려지고
> 스펙이 그 결정을 흡수한 것이다(§7).

### 3.1 흐름

```
L1 제안 → remediation_l2 propose(원장에 등록 + 승인 카드)
   → 사람이 서버 셸에서  approve <id>          … dry-run(기본, 부작용 0)
   → 사람이 서버 셸에서  approve <id> --apply  … 런북을 다시 읽어 실행
   → rc·소요·출력 꼬리·롤백 안내를 감사 원장에 append
```

- **사람의 타이핑 = 헌장 "사람이 적용"**(§11). LLM은 트리거하지 않는다.
- 실행 주체는 **LLM이 아니라 결정론 실행기**. 실행기는 `proposal_id` 만 받고
  **명령 문자열은 인자로 받지 않는다** — 런북 파일에서 다시 읽는다.
- **데몬이 없다.** 리스너·타이머·큐·워커 루프 0. 사람이 치지 않으면 실행을 기다리는
  프로세스가 존재하지 않는다(ADR-0026 §C1의 핵심 논증).

### 3.2 승인 게이트의 형해화 방지

승인 피로로 습관적 승인이 되면 human-in-the-loop이 무너진다(§8). 승인 카드에 **강제 표기**
(5필드 중 하나라도 비면 `CardError` — 카드가 없으면 승인도 없다):

| 필드 | 내용 | 비면 어떻게 되나 |
| --- | --- | --- |
| **무엇을** | `runbook_id` / `action_id` / 제목 | 승인 불가 |
| **왜** | 근거번호 `[n] 파일:행` (L1이 서버검증한 인용) | 근거 0개 = 승인 불가(§0-3) |
| **영향범위** | 대상 노드/서비스 · risk · tier · 명령 수 | 대상 미상 = 승인 불가 |
| **롤백** | 선언된 롤백 명령, 없으면 **"미선언"을 명시** + 런북 포인터 | 승인 불가 |
| **dry-run** | 실행될 명령 **전문** | 명령 0개 = 승인 불가 |

- `--dry-run` 이 기본이고, dry-run 실행 자체가 원장에 남는다 — **"보지 않고 승인했는가"가
  사후에 드러난다.** 형해화를 없앨 수는 없지만 기록되게 만들 수는 있다.
- 고위험(`risk: high`)은 2단계 확인이 아니라 **거부**다. 사람이 직접 하라.

### 3.3 승인 채널 — **서버 CLI 단일화**(Slack 버튼 미채택)

초안의 "1차 Slack 버튼 / 폴백 CLI"를 **뒤집었다**. 근거 전문은 ADR-0026 "스펙을 뒤집은 판단":

- Slack 인터랙티브 버튼은 **Slack이 우리에게 POST할 공개 엔드포인트**를 요구한다. 이 망의
  인바운드는 Cloudflare 터널 하나뿐이고 정책은 "외부 진입 경로의 존재 자체를 알리지 않는다"다.
- 승인은 **한 달에 몇 번**인데 엔드포인트는 **24시간** 노출된다 — 비대칭이 크다.
- 승인 권한이 **사외 신원 체계(Slack 워크스페이스)** 로 나가는 것은 §14(인증=Cloudflare Access)와 어긋난다.
- **버튼은 마찰을 없애서 §3.2의 방어(읽게 만드는 마찰)를 함께 없앤다.**
- Socket Mode(아웃바운드 WebSocket)는 인바운드 문제만 풀고 나머지 둘은 그대로 → 기각.
  단, **인바운드 없이 버튼이 가능하다는 사실은 재검토 시 첫 후보**로 기록해 둔다.

받아들이는 손실: **폰에서 승인할 수 없다.** L2가 사는 것은 속도가 아니라 **정확성(드리프트
검증)과 추적성(감사 원장)**이다. Slack은 ADR-0018·0025 범위 그대로 **단방향 통지 전용**이고,
나가는 필드는 `slack_fields()` 한 함수로 좁힌다. **control plane은 관제 대상 노드 밖**
(data05 터널 런북을 data05에서 실행 금지 — Facebook 2021)이라는 §C4는 그대로 유효하다.

### 3.4 실행기 요구사항

- **명령의 유일한 출처는 런북 파일.** 실행 직전 디스크에서 다시 읽는다.
- **실행 전 재검증(전부 fail-closed):** 런북 SHA-256 · 조치 지문 · 명령 일치 · tier ≥ 2 ·
  신선도(stale 거부) · `risk ∈ {low, medium}` · `reversible: true` · `idempotent: true` ·
  파괴 동사 부재 · 명령 치환(`$(`·백틱·`${`) 부재 · **본문 코드블록 근거성(A7 런타임 재확인)**.
- **실행 지점은 한 곳**(`_run_one` 안의 `subprocess.run` 1회 — 게이트 M5). 환경은 물려받지 않고
  상수 화이트리스트로 만든다. timeout · rc · 소요 · 출력 꼬리를 기록한다.
- **부분 실패에서 멈춘다.** 앞 명령이 실패했는데 뒤를 계속 돌리면 런북이 기술한 절차가 아니다.
- **자동 롤백은 하지 않는다**(L3 소관). 실패하면 롤백 안내를 출력하고 멈춘다.
- **멱등**: 같은 `proposal_id` 는 두 번 실행되지 않는다. 제안은 TTL(기본 1h) 뒤 만료된다.

### 3.5 AC (기계 검증 — `check-remediation-l2.sh` M1~M9 + 유닛 50건)

- **AC-L2-1** 승인 없이는 실행 0: 승인 이벤트가 없는 `proposal_id` 를 `apply=True` 로 실행하면
  거부되고 부작용이 0이다. → `test_remediation_l2.TestApprovalGate` + 게이트 M4.
- **AC-L2-2** 승인 카드 필수 5종(무엇/왜/영향/롤백/dry-run) 누락 시 카드 생성 실패 →
  승인 불가. → `TestApprovalCard`.
- **AC-L2-3** **CLI가 유일 경로**: `approve <id> --apply` 로 실행이 트리거되고, Slack 경로 없이
  전 생애주기가 완결된다. → `TestDryRun.test_cli_apply_flag_executes`.
- **AC-L2-4** 감사: 1회 실행마다 원장에 `proposal_id`·`approver`·`approved_by`·`approved_at`·
  `runbook_id`·`action_id`·`runbook_sha256`·`results`(rc 포함)·`rollback`·`outcome` 존재.
  원장은 append-only(O_APPEND+fsync·0600)이고 수정 API가 없다. → `TestLedger` + 게이트 M7.
- **AC-L2-5** 유출 필드 화이트리스트: `slack_fields()` 산출에 `user`/`pid`/`cmdline`/`instance`/
  `commands`/`runbook_path` 키가 **없다**(블랙리스트가 아니라 화이트리스트라 새 필드도 안 샌다).
  → `TestEgressFields`.
- **AC-L2-6**(신규) **드리프트 거부**: 제안 이후 런북 본문·조치 라벨·명령이 바뀌면 실행 0.
  삭제된 런북도 마찬가지. → `TestDrift`.
- **AC-L2-7**(신규) **dry-run 무해**: 기본 경로에서 부작용이 실제로 0이다(마커 파일 미생성).
  같은 제안이 `--apply` 에서는 실제로 실행된다(반증). → `TestDryRun`.
- **AC-L2-8**(신규) **화이트리스트 밖 거부**: tier ≤ 1 · risk high · 비가역 · 비멱등 ·
  파괴 동사 · 명령 치환 · 미근거 명령 · stale 런북에서 실행 0. → `TestPolicyRefusal`.
- **AC-L2-9**(신규) **경계**: L1 모듈이 L2를 참조하지 않는다(`check-remediation-l1.sh` L8) ·
  실행 지점이 정확히 1곳(M5) · 실행기가 명령 파라미터를 받지 않는다(M2) · 데몬 0(M6).

> 유닛 테스트는 **실행 경로를 실제로 탄다**(`echo`·`touch`·`sleep`·`/bin/false`, 대상은 tempdir).
> 모킹으로 대체하면 정작 검사해야 할 그 한 줄을 안 보게 된다. 외부 통신 0 · 라이브 영향 0.

### 3.6 초안에서 뒤집힌 판단 3건 (기록)

| # | 초안 | 확정 | 사유 |
| --- | --- | --- | --- |
| 1 | Slack 버튼 1차 / CLI 폴백 | **CLI 단일** | 공개 인바운드 1개 + 사외 신원 + 승인 마찰 제거의 비용 > 편의(ADR-0026) |
| 2 | 감사 = OpenSearch `keiwi-remediation-*` | **로컬 append-only JSONL**(OpenSearch는 filebeat 파생 뷰) | L2 1호 후보가 `LogIngestStalled` 다 — 로그 파이프라인을 고치는 조치의 감사를 그 파이프라인으로 보내는 자기모순(ADR-0025의 "게시 전 예약" 기각과 같은 논리) |
| 3 | 롤백은 대칭 `rollback_ref` 필수 | **롤백 필드는 필수, 내용은 "선언된 명령 또는 미선언 명시"** | 우리 tier≥2 조치엔 역명령이 없다("재시작"의 역은 없다). 대칭 롤백은 §4.1의 **L3 조건**이고, L2에서 그것을 필수로 하면 대상이 0개가 되어 규칙이 아니라 장식이 된다. 대신 미선언 사실을 카드·원장에 **드러낸다** |

---

## 4. L3 — 사전승인 안전조치 자동 (ADR-0027(신설 예정) 게이트)

> [!WARNING]
> L3는 사람 클릭 없이 결정론 워커가 라이브를 바꾼다 — **진짜 새 결정**이다. **ADR-0027(신설 예정) 승인 전엔
