...
- [ ] **[02 §7](./02-brand-color-roles.md)의 8단계로 브랜드 3단을 다시 계산.** 건너뛰면 포커스 링이 1.4.11 위반.
...
2. `success-*`/`info-*` 사용처를 찾아 무채색 + 형태 부호화로 바꾼다(**여기가 결함 ③이 터지는 지점** — 상태 구분이
   소실되지 않았는지 반드시 확인).
   ⚠️ **차트 시리즈 색은 이 단계의 대상이 아니다.** 무채색화는 **상태**에만 적용한다. 시리즈까지 걷어내면
   다중 시계열이 판독 불가가 된다(**결함 ④**). 시리즈는 `series-1~6`으로 **고정 배정**하고 하위 구분은
   선 스타일로 한다([02 §5](./02-brand-color-roles.md)).
