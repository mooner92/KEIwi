...
| **B0.8** | 콘텐츠 우선 레이아웃(섹션 헤더 제거·슬림 툴바·초콤팩트 카드·새탭 링크 탭행 흡수·H1 24 — 목표 임베드 ≥65%) — 근거 [00 관제(NOC) 밀도 예외](./00-principles.md) | 🔄 병렬 구현 중 (2026-07-03) |
| **B0.9** | 로그 워크벤치 v2(임베드:어시스턴트 **1:1** · 드로어 **필터 칩** · 패널 헤더 **브랜드 틱**·active 행 액센트 바) — 스펙 [logs-assistant v2](../logs-assistant/spec.md) | ⬜ 대기 (2026-07-04 스펙) |
...
- **카드 콤팩트**(사용자 피드백): 노드 카드 밀도 회귀(패딩 10px) — B0.5의 여백 확대 일부 되돌림, 위계(제목 17px·상태·배지)는 유지. ⚠️ [03] 카드 패딩 규격(16~24) 예외 — 아래 모호점 ③(→ [00 관제(NOC) 밀도 예외](./00-principles.md)로 정당화됨).

### 감사가 드러낸 스펙 모호점 (2026-07-03 전건 해소)
1. **배지 4px vs 칩 pill**: [03] 공통 반경 줄은 "칩·태그·배지 **4**"인데 Tag/Chip 절은 "**pill**(radius=높이)" — 상충. 적용 해석 = **비인터랙티브 배지 4px · 인터랙티브 칩(어시스턴트 예시칩 등) pill**("Badge=비인터랙티브 / Tag=인터랙티브" 구분 준용). → **해소**(2026-07-03, [03](./03-components.md) 공통 반경 줄 정정 + [01](./01-foundations.md) 반경 표 정합).
2. **font-weight 500/600 관행**: [01]은 400(label)·700(제목)만 정의 — 코드는 `font-medium`(500, 배지·보조 라벨)·`font-semibold`(600, 카드/패널 제목) 관행. → **해소**(2026-07-03, [01](./01-foundations.md) 굵기 규약 표 명문화 — 400 본문/500 보조 라벨·배지/600 카드·패널 제목/700 페이지 제목·탭·강조, 500/600=synthetic weight 주기 + [03] 공통 폰트 줄 연결).
3. **관제 밀도 예외**: [00]은 "표/목록 여백 축소 허용(본문 크기·줄간격·포커스는 불축소)"만 규정 — 카드 패딩(규격 16~24 vs 노드 카드 10px)·패널 헤더 등 **관제(NOC) 밀도 예외의 적용 범위** 미규정. → **해소**(2026-07-03, [00](./00-principles.md) "관제(NOC) 밀도 예외" 명문화 — H1 24·섹션 헤더 aria-label 생략·카드 패딩 8~12·콘텐츠 우선 ≥60%·불축소 항목 재확인).
