...
> [!IMPORTANT]
> **상태: 제안(Draft).** 외부 SaaS 의존을 새로 도입하는 결정이므로 게이트(§9 질문 답변 + ADR-0028(신설 예정) 채택) 통과 전
...
그럼에도 이것은 KEIwi 최초의 **상시 외부 SaaS 의존**이므로 ADR-0028(신설 예정)으로 명시 채택해야 한다
...
- **outside-in 프로브는 범위 밖** — 외부 도메인(터널 앞단)을 밖에서 찔러 "터널·콘솔이 실제로 응답하나"를 보는
...
- [ ] **Q1.** 외부 SaaS 의존(생존 신호 1비트)을 승인하는가? → 승인 시 ADR-0028(신설 예정) 작성
...
- [ ] **TW-1** (S) ADR-0028(신설 예정) 작성 — 외부 생존 신호 채택, egress 예외 범위 한정
