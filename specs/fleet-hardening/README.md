...
- **내부망 정보의 공개 레포 노출 억제** — 이 레포는 **PUBLIC**이고(spec §5.1) 런북·AC·인벤토리는 이미 `192.0.2.1N`·`:<SSH_PORT>`·계정명·`localhost:9200`을 담고 있다. 축3이 만드는 런북 6종도 같은 관행을 따른다. ~~**이건 이 스펙이 새로 만든 위험이 아니라 이미 합의된 트레이드오프**이므로 여기 명문화만 하고 바꾸지 않는다~~ → **2026-08-19 뒤집혔다**: 공공 GitLab 등록으로 청중이 바뀌어 문서의 IP·SSH 포트를 자리표시자로 전환했다([ADR-0029](../../docs/decisions/0029-internal-network-info-in-public-repo.md), 게이트 P5). 운영 설정은 후속(축5 S2의 사설 IP 금지는 `apps/console/src` 런타임 소스 한정 — 배포 산출물이 환경에 못 박히는 것을 막는 규칙이지 정보 은닉 규칙이 아니다). 재검토가 필요하면 ADR-0023에 근거를 남기고 별건으로 연다.
