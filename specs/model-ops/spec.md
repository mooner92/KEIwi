...
- **자동 롤백·사후 헬스체크 기반 자동 조치** — L3 영역(ADR-0029 **신설 예정** 이후).
...
- **v1.5 (ADR-0029 — 신설 예정 — 승인 후)**: 콘솔 버튼 실행. 안전장치 4종을 조건으로 — ① Cloudflare Access 신원을 원장 `approver`에 기록 ② 확인 마찰(모델 slug 타이핑) ③ 대상은 `vllm-*` 유닛 화이트리스트만(범용 명령 실행기 금지 — ADR-0026 기각 사유 계승) ④ 실행 주체는 콘솔이 아니라 **로컬 실행기 소켓**(콘솔 프로세스에 sudo를 주지 않는다).
...
- **Q1 (v1.5 게이트)**: 콘솔 버튼 실행(ADR-0029 신설 예정)을 승인할 것인가, v1 복붙 방식을 유지할 것인가? — §7 안전장치 4종이 전제.
...
| ADR-0029 (신설 예정) | 콘솔 개시 조치의 신원·마찰·화이트리스트 — Q1 승인 시 작성 |
