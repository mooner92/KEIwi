...
## 2. 브랜드 정책 — **표준형 (정부 블루)** (결정: ADR-0007 개정)

> ⚠️ 개정(2026-06-27): 초기 확장형(green Primary)을 **표준형으로 번복**. 원본 KRDS 대비 인상이 약해 사용자가 표준형 선택. 아래는 표준형 기준.

**Primary = KRDS 정부 블루**(`#256ef4`, `--krds-color-light-primary-50`). 버튼·링크·활성 탭·포커스·내비 활성에 사용. **KEIwi green은 로고(BrandMark)·소량 액센트로만** 강등하고 **Primary·상태색에는 쓰지 않는다**(§17). 헤더는 흰 배경 + 정부 공식 식별 배너.
