...
# ③ data01로 전송(포트 <SSH_PORT> · 계정은 env, 레포에 적지 않는다 §13)
scp -P <SSH_PORT> ./smartctl "$KEIWI_USER_DATA01@192.0.2.11:/tmp/smartctl-7.4"

# ④ 설치(사람, sudo)
ssh -p <SSH_PORT> "$KEIWI_USER_DATA01@192.0.2.11" 'sudo install -o root -g root -m 0755 \
...
```ini
data01 ansible_host=192.0.2.11 ansible_user="{{ ... }}" fleet_node=data01 \
...
ssh -p <SSH_PORT> "$KEIWI_USER_DATA01@192.0.2.11" 'sudo /tmp/smartctl-7.4 --json -d cciss,0 /dev/sg0'
```
