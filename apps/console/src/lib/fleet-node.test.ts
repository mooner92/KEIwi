...
const NODES = [
  { id: "data03", ip: "192.168.1.103" },
  { id: "data04", ip: "192.168.1.104" },
  { id: "data05", ip: "192.168.1.105" },
];
...
describe("normalizeFleetNode (딥링크 node 파라미터 정규화)", () => {
  ...
  it("ip → id", () => {
    expect(normalizeFleetNode("192.168.1.104", NODES)).toBe("data04");
  });
  it("ip:port (Grafana instance 라벨) → id", () => {
    expect(normalizeFleetNode("192.168.1.104:9100", NODES)).toBe("data04");
    expect(normalizeFleetNode("192.168.1.103:9400", NODES)).toBe("data03");
  });
  it("미지 입력 → undefined", () => {
