...
export const CHANGELOG: ChangelogEntry[] = [
  ...
  // ── 2026-08-05 ──────────────────────────────────────────────
  {
    ...
    type: "신규",
    scope: "alerting",
    title: "L2 승인 실행기 — 사람이 승인해야만 도는 조치",
    summary:
      "자율 사다리 L2(ADR-0026): 알림→L1 제안을 사람이 CLI로 승인해야 실행되는 결정론 실행기. 상태를 바꾸는 지점은 remediation_l2.py의 subprocess.run 정확히 1회뿐이고 게이트가 그것을 강제한다 — 사다리의 경계가 문장이 아니라 파일 경계다. dry-run 기본 · 실행 전 7단계 재검증(승인 이벤트·런북 SHA·명령 근거성·정책) · append-only 감사 원장. fail-open 3건 차단: 무기록 실행(기록을 먼저 남기고 그 append가 실패하면 실행 0), 손상 원장(깨진 줄 하나라도 있으면 거부), TOCTOU(flock). 콘솔 버튼 실행은 명시적으로 기각 — 공개 엔드포인트·사외 신원·승인 피로.",
  },
  // ── 2026-08-06 ──────────────────────────────────────────────
  {
    date: "2026-08-06",
    ...
    scope: "infra",
    incident: true,
    title: "data05 드라이버 mismatch 수복 — 그리고 증거를 잃었다",
    summary:
      "NVIDIA 커널모듈↔유저스페이스 불일치(595.71.05 vs 595.84)로 nvidia-smi가 exit 18, 신규 GPU 프로세스가 전부 기동 실패하던 상태를 재부팅으로 수복. 사후 검증 통과(CDI 재생성·gpu_vram 복귀·vLLM 기동). 다만 **재부팅이 탐지 배포보다 먼저 일어나** mismatch 1→0 전이가 시계열에 남지 않았다 — 스펙이 \"되돌릴 수 없는 순서 제약\"으로 경고한 그 위반이 실제로 발생했고, AC-3-2는 이 사건으로는 영구 미달성이다. 지금의 =0은 \"고쳤다\"가 아니라 \"현재 정합\"만 뜻한다. 런북 nvidia-driver-mismatch 신설 · inventory에 driver·kernel_module 실측 필드. 부수 발견: 8003 모델이 Qwen2.5-Coder-32B로 교체돼 어시스턴트 502, PM2 startup 미등록.",
  },
  {
    date: "2026-08-06",
    shas: ["4744526", "f76aabf", "f9a5f65"],
    type: "신규",
    scope: "console",
    title: "모델 운영(/models) — 서빙 가시화 + VRAM 사전판정",
    summary:
      "\"어느 GPU에 뭐가 떠 있고, 이 모델을 띄울 수 있나\"를 콘솔에서 답한다. 노드×GPU 격자(모델·포트·소유자·VRAM) + 노드별 설치 카탈로그 + 4단 판정(가능/빠듯/불가/판정불가). 판정식은 vLLM이 가중치가 아니라 gpu-memory-utilization만큼 **예약**하는 특성을 반영한 2겹이고, \"빠듯\"에는 권장 util을 수치로 제시한다. 메트릭 결손은 판정불가(ADR-0013 정직성). 카탈로그는 roles/model-catalog가 각 노드에서 디스크 스캔+ollama list를 textfile로 노출 — 신규 포트·scrape job·ufw 규칙 0(node-hygiene 배관 재사용). 시각 QA가 오판을 잡았다: 61 GiB 모델이 유휴 44 GiB GPU에서 \"가능\"으로 나오던 검사 순서 결함(회귀 테스트 2건 추가).",
  },
  // ── 2026-08-07 ──────────────────────────────────────────────
  {
    date: "2026-08-07",
    ...
    type: "수정",
    scope: "console",
    title: "Overview 탭 무반응 — URL 구동으로 하이드레이션 의존 제거",
    summary:
      "탭 활성 상태가 useState에만 있어 하이드레이션이 실패하면 통째로 죽었다(SSR HTML은 정상이라 화면은 멀쩡하고 클릭만 무반응). 이 파일은 같은 계열의 회귀를 이미 겪었고 \"부재가 잘못된 상태보다 나쁜 실패\"라는 교훈이 주석에 남아 있었는데, 임베드에 적용한 그 교훈이 탭에는 없었다. 활성 상태를 ?tab= URL로 옮겨 탭을 링크로 만들었다 — JS OFF에서도 전환·iframe 교체 동작 확인. 부수: 노드를 고르면 탭이 시스템으로 튕기던 문제도 해소(딥링크·뒤로가기 획득).",
  },
  // ── 2026-08-10 ──────────────────────────────────────────────
  {
    date: "2026-08-10",
    ...
    type: "문서",
    scope: "infra",
    title: "디스크 압박의 정체 — RAID6 오인식이 아니었다",
    summary:
      "\"22TB인데 90%가 찼을 리 없다\"는 의심이 맞았다. 알림이 가리킨 것은 배열이 아니라 **447GB OS 디스크**였고, RAID6(22T)는 3%였다. /home이 작은 부팅 디스크에 얹혀 연구 데이터가 거기 쌓인 것. data04는 −33 GiB/7d로 4일 뒤 소진, data05는 관제 스택 호스트라 채워지면 Prometheus·OpenSearch·Grafana가 동시에 멈춘다. 이전 런북 작성 — 전수조사에서 tmux 3명·창 11개가 최장 3월부터 상주 중임을 확인해 **사용자별 bind mount**로 설계를 바꿨다(일괄 이전은 전원 세션을 동시에 끊어야 해 조율 실패 시 작업 창이 통째로 날아간다). 부수 발견: data05 홈 5개가 755로 열려 있어 \"홈은 보호된다\"는 전제가 data04에서만 사실이었다.",
  },
  // ── 2026-08-12 ──────────────────────────────────────────────
  {
    date: "2026-08-12",
    ...
    type: "신규",
    scope: "console",
    title: "로그분석 LLM을 ollama로 복구 — thinking을 못 끄는 함정",
    summary:
      "vLLM(:8003)을 내린 뒤 죽어 있던 어시스턴트를 data05 상주 ollama qwen3.5:9B로 연결. **OpenAI 호환 경로로는 불가능**하다는 것이 실측으로 확인됐다: ollama의 /v1/chat/completions는 think 파라미터를 무시하고 프롬프트 /no_think도 먹지 않아, 응답이 reasoning만 채운 채 content=\"\"로 잘린다(max_tokens 2000에서도 사고과정이 예산을 전부 소진). 네이티브 /api/chat + think:false만 정상 응답한다. VLLM_BACKEND(openai|ollama)로 분기하되 **자동 추측하지 않는다** — URL만으로 구분 불가이고 추측은 조용히 틀리는 쪽을 고른다. content가 비고 thinking만 있으면 원인을 적어 throw(빈 답변 오진 방지).",
  },
  {
    date: "2026-08-12",
    shas: ["294701c", "e860b5a"],
    type: "수정",
    scope: "console",
    incident: true,
    title: "거짓 초록 3건 — 수집 실패가 \"정상\"으로 보이던 자리",
    summary:
      "data03이 GPU를 17 GiB 쓰는데 서비스 탭은 \"GPU에 적재된 프로세스 없음\"이라 **노는 서버로 읽혔다**. 원인은 드라이버 mismatch 재발 — gpu-model-exporter는 nvidia-smi에 의존해 0건이 되는데 DCGM은 커널모듈 값을 읽어 정상 보고하니 대시보드만 초록이었다. 수집기 자체 신호가 미배포라 **가진 두 데이터의 모순**으로 판정한다(VRAM 사용 중인데 모델 0건 = 수집 실패). 이어진 코드 점검이 같은 실패모드를 두 곳 더 찾았다: 현재 신호 패널·로그 워크벤치가 OpenSearch 조회 실패를 \"지금 신호 없음(정상)\"으로 접고 있었다 — 인시던트 탐지의 1차 진입점이다. 첫 판정 구현의 자체 버그도 교정(노드 임계에 플릿 합계를 먹여 오탐, 동시에 노드별 실패는 미탐).",
  },
  {
    date: "2026-08-12",
    shas: ["f1ff82b"],
    type: "수정",
    scope: "console",
    title: "LAN IP로 열면 클릭이 전부 죽던 함정",
    summary:
      "\"탭이 안 눌린다\"→\"테마 토글이 안 된다\"→\"분석 버튼이 안 눌린다\" 신고 3건이 전부 같은 원인이었다. dev 서버를 LAN IP로 열면 Next 15.2+의 allowedDevOrigins가 webpack-hmr WebSocket을 거부하고, 그 여파로 하이드레이션이 통째로 죽어 모든 클라이언트 인터랙션이 무반응이 된다. 진단이 어려운 이유가 조치의 이유다 — SSR HTML도 서버 API도 정상 200이라 화면·서버 로그 어디에도 원인이 안 드러나고, 헤드리스 검증조차 localhost로 돌리면 통과한다(실제로 두 번 재현에 실패했다). 프로덕션(next start)은 HMR이 없어 무관. 메커니즘은 이미 있었고 값만 비어 있었으나 .env.local은 커밋되지 않아 새 워크트리마다 재발하므로, dev 기동 시 경고를 찍고 docs/testing.md에 증상→원인→조치를 남겼다.",
  },
  {
    date: "2026-08-12",
    shas: ["8080abd"],
    type: "신규",
    scope: "console",
    title: "코드 그래프(/graph) — 서버 렌더 SVG",
    summary:
      "graphify의 인터랙티브 시각화(graph.html, vis-network — 1,419노드·2,482엣지·커뮤니티 110)를 Grafana와 같은 액자 패턴으로 임베드한다. 드래그·줌·검색·커뮤니티 토글·노드 인스펙터. **수제 SVG를 먼저 만들었다가 버렸다** — 정적 점구름은 읽히지 않았고, graphify의 exact-dedup이 병합한 심볼을 잘못 귀속해 가짜 의존(model-ops-view→prometheus.test.ts)까지 그렸다. 재구현 금지 원칙(§I-2)을 콘솔 자신에게 적용한 사례. 콘솔이 얹는 것은 판정뿐: 파일 136·파일 간 의존 128·고립 50(셸·설정)·허브·신선도(추출 커밋 ≠ HEAD면 재생성 안내). 병합 심볼 간선은 버린다 — 없는 의존을 그리는 것보다 빠뜨리는 쪽이 낫다.",
  },
  {
    date: "2026-08-13",
    shas: ["cdbe297"],
    type: "신규",
    scope: "console",
    title: "서비스 탭 ↔ 플릿 위키 연결 + 문서 그래프",
    summary:
      "포트 행이 위키 문서로 이어진다 — projects/*.md의 frontmatter(node·ports)에서 (노드:포트)→문서 색인을 만들어, 매칭 행엔 '위키' 링크(행의 어시스턴트 링크와 중첩되지 않게 형제 앵커), 문서 없는 행엔 '미등록' 배지를 단다. 배지는 **위키가 커버하는 노드에서만** — scout가 data05에만 배포된 상태에서 전 노드에 칠했더니 배지가 431회 도배됐고(신호 가치 0), 커버 노드로 좁혀 37회가 됐다. /wiki에는 그래프 보기가 생겼다: 서버→계정→프로젝트 3컬럼 SVG, 간선은 슬러그 구조에서 결정론으로 유도(문서 재파싱·물리 시뮬레이션·신규 의존성 없음).",
  },
];
