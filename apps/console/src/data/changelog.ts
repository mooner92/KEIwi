...
export const CHANGELOG: ChangelogEntry[] = [
  ...
  },
  {
    ...
    summary:
      "graphify(extract --code-only)의 AST 산출물을 파일 단위 의존 그래프로 그린다. LLM 호출 0·egress 0·신규 npm 의존성 0. 심볼 1,419노드를 파일 단위(136파일·128의존)로 접고, 파일 판별은 contains 간선의 출발점으로 한다 — 확장자로 판별하지 않는다(확장자 없는 스크립트가 실재). **배치를 두 번 만들었다**: 커뮤니티 원형 배치가 먼저였는데 실측에서 커뮤니티가 80개로 쪼개져(대부분 1~2개) 그림이 \"큰 고리\"로 뭉개졌다 — 해바라기 배치로 교체해 연결 많은 파일이 중심에 오게 했다. 고립 파일 50개는 그리지 않고 개수만 알린다. 서버 렌더 SVG라 JS 없이도 보인다 — 하이드레이션이 죽어 위젯이 무반응이 된 사고를 겪은 콘솔에서 읽기 전용 그림을 클라이언트 상태에 걸 이유가 없다. 산출물 부재는 \"미생성\"으로 표기하고 생성 명령을 안내한다.",
  },
];
