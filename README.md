...
| **M2** | 통합 로그 (OpenSearch, Grafana 단일 콘솔 · 분류·신호우선) | ✅ 라이브 |
| **M3** | 여유 리소스("free" 판정 + 작업배치) → **Overview에 흡수**(M1 고도화) | 재배치 |
| **M4** | 장애 추적·시각화 | **보류**(M2 신호뷰로 충족, 필요시 annotation) |
| **M5** | 크리티컬 에러 알림 (에러→책임자 이메일, push) | 후순위(M1/M2 심화 후) |
| **다음** | **M1/M2 고도화** — Overview 여유리소스·메트릭↔로그 드릴다운·모델↔GPU 노출 등 | 진행 예정 |
